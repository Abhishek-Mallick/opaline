...
export function Tile({ name }: { name: string }) {
  ...
  return (
    ...
    >
      <Stage name={name} className="min-h-[280px] flex-1" />
      <footer className="flex items-center gap-2 border-t border-border py-2.5 pr-3 pl-5">
        <Link
...
export function Stage({ name, className }: { name: string; className?: string }) {
  const meta = demoMeta[name] ?? {}
  return (
    <div
      className={cn(
